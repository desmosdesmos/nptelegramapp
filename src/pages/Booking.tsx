...
const ServiceCheckboxOption: React.FC<{
  ...
  return (
    <div
      className={`w-full p-4 rounded-2xl transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)]
        ${isSelected
    ...
    >
      <div className="flex w-full items-center">
        ...
        >
          <div className={`w-5 h-5 flex-shrink-0 border-2 rounded-md flex items-center justify-center mr-3 ${isSelected ? 'border-blue-500 bg-blue-500/20' : 'border-white/30'}`}>
            {isSelected && <div className="w-2 h-2 rounded-sm bg-blue-500 shadow-blue-500/50" />}
          </div>

          <div className="flex-1 min-w-0 flex flex-col">
            <div className="flex items-center gap-2 min-w-0">
              {service.icon && <span className="text-lg flex-shrink-0">{service.icon}</span>}
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-baseline gap-x-2">
                  <span className="font-medium text-white truncate flex-1 min-w-0">
                    {service.name}
                  </span>
                  {service.unitLabel && <span className="text-[10px] text-gray-400 whitespace-nowrap flex-shrink-0 self-center">{service.unitLabel}</span>}
                </div>
              </div>
            </div>
            {service.description && <p className="text-xs text-gray-400 mt-1 pl-0" dangerouslySetInnerHTML={{ __html: service.description.replace(/\n/g, '<br />') }}></p>}
          </div>
        </button>
        ...
        <div className="flex flex-col items-end ml-2">
          ...
          <span className="text-base font-semibold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-400 whitespace-nowrap">
            {showQuantityControl ? (service.price * quantity).toLocaleString('ru-RU') : service.price.toLocaleString('ru-RU')}<span className="whitespace-nowrap">&nbsp;₽</span>
          </span>
        </div>
      </div>
    </div>
