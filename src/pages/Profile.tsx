...
const Profile: React.FC<ProfileProps> = ({ onNavigate }) => {
  ...
  const copyCode = () => {
    navigator.clipboard.writeText('AUTO2024');
    setCopied(true);
  ...
  const shareCode = () => {
    if (navigator.share) {
      navigator.share({
        ...
        text: 'Используй промокод AUTO2024 и получи скидку 500₽ на первую мойку!',
    ...
    <div className='w-full min-h-screen flex flex-col p-6 pt-12 pb-44 bg-black text-white'>
      ...
      <div className="w-full p-6 mb-8 rounded-2xl bg-gradient-to-br from-amber-300 via-orange-400 to-amber-500 shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-black/80 mb-4">на первую мойку, и получи бонусы себе</p>
          
          <div className="bg-white/20 backdrop-blur-sm border-2 border-dashed border-white/30 rounded-xl p-4 mb-6">
            <p className="text-center text-lg font-bold text-black">AUTO2024</p>
          </div>
      ...
      <div className="w-full">
        ...
        <div className="space-y-4">
          <button className="w-full p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md flex items-center gap-3 hover:bg-white/10 transition-colors">
